<script lang="ts">
	/**
	 * SIMPLIFIED VERSION - No intermediate state, work directly with form fields!
	 *
	 * This demonstrates the pattern:
	 * - Read with: field.value()
	 * - Write with: field.set(value)
	 * - For native inputs: {...field.as('text')}
	 * - For custom components: value={} + onchange={}
	 */
	...
	// Reactive: Get current selected seniority levels from form (parse JSON array)
	const selectedSeniority = $derived.by(() => {
		const value = fields.job.seniority.value();
		if (!value) return [];
		try {
...
<Field.Set>
	...
	<Field.Group class="@container/field-group">
						...
						<Field.Field orientation="horizontal">
							<Checkbox
								...
								onchange={(e) => {
									...
									fields.job.seniority.set(JSON.stringify(newSeniority));
								}}
							/>
		...
		<!-- Calendar example - we might need UI state for the CalendarDate object -->
		<Field.Field
			orientation="responsive"
